...
<script setup>
	...
	function getGiftDetailsCheckbox(eventId) {
		if(giftedSubsDetailsCheckboxMap.value.has(eventId)) {
			return giftedSubsDetailsCheckboxMap.value.get(eventId);
		}
		const newState = settings.giftSubs_autoExpand;
		giftedSubsDetailsCheckboxMap.value.set(eventId, newState);
		return newState;
	}
